...
/// Get state directory (for logs, PIDs, runtime state)
...
pub fn state_dir() -> Result<PathBuf> {
    if is_custom_paths_allowed() {
        if let Ok(custom_path) = std::env::var("LOCALAPPDATA") {
            warn!(
                "KODEGEN_ALLOW_CUSTOM_PATHS is enabled - bypassing validation for LOCALAPPDATA (UNSAFE)"
            );
            return Ok(PathBuf::from(custom_path).join("kodegen").join("state"));
        }
    }
    
    let validated = std::env::var("LOCALAPPDATA")
        .ok()
        .and_then(|p| {
            match validate_env_path("LOCALAPPDATA", &p) {
                Ok(validated) => Some(validated),
                Err(e) => {
                    warn!("Invalid LOCALAPPDATA environment variable: {}. Falling back to system default.", e);
                    None
                }
            }
        });
    ...
    if let Some(validated_path) = validated {
        Ok(validated_path.join("kodegen").join("state"))
    } else {
        dirs::data_local_dir()
            .map(|d| d.join("kodegen").join("state"))
...
#[cfg(target_os = "macos")]
pub fn state_dir() -> Result<PathBuf> {
    // macOS: Use Application Support with state subdirectory (XDG-like pattern)
    if is_custom_paths_allowed()
        && let Ok(custom_path) = std::env::var("XDG_STATE_HOME") {
            warn!(
                "KODEGEN_ALLOW_CUSTOM_PATHS is enabled - bypassing validation for XDG_STATE_HOME (UNSAFE)"
            );
            return Ok(PathBuf::from(custom_path).join("kodegen"));
        }
    
    let validated = std::env::var("XDG_STATE_HOME")
        .ok()
        .and_then(|p| {
            match validate_env_path("XDG_STATE_HOME", &p) {
                Ok(validated) => Some(validated),
                Err(e) => {
                    warn!("Invalid XDG_STATE_HOME environment variable: {}. Falling back to system default.", e);
                    None
                }
            }
        });
...
pub fn state_dir() -> Result<PathBuf> {
    if is_custom_paths_allowed() {
        if let Ok(custom_path) = std::env::var("XDG_STATE_HOME") {
            warn!(
                "KODEGEN_ALLOW_CUSTOM_PATHS is enabled - bypassing validation for XDG_STATE_HOME (UNSAFE)"
            );
            return Ok(PathBuf::from(custom_path).join("kodegen"));
    ...
    if let Some(validated_path) = validated {
        ...
        dirs::state_dir()
            .map(|d| d.join("kodegen"))
            .or_else(|| dirs::home_dir().map(|h| h.join(".local/state/kodegen")))
            .ok_or_else(|| anyhow!("Cannot determine state directory"))
    }
}
...
pub fn data_dir() -> Result<PathBuf> {
    if is_custom_paths_allowed() {
        if let Ok(custom_path) = std::env::var("LOCALAPPDATA") {
            ...
            return Ok(PathBuf::from(custom_path).join("kodegen").join("data"));
        }
    }
    ...
    if let Some(validated_path) = validated {
        Ok(validated_path.join("kodegen").join("data"))
    } else {
        dirs::data_local_dir()
            .map(|d| d.join("kodegen").join("data"))
...
pub fn data_dir() -> Result<PathBuf> {
    if is_custom_paths_allowed()
        && let Ok(custom_path) = std::env::var("XDG_DATA_HOME") {
            warn!(
                "KODEGEN_ALLOW_CUSTOM_PATHS is enabled - bypassing validation for XDG_DATA_HOME (UNSAFE)"
            );
            return Ok(PathBuf::from(custom_path).join("kodegen"));
        }
    
    let validated = std::env::var("XDG_DATA_HOME")
        .ok()
        .and_then(|p| {
            match validate_env_path("XDG_DATA_HOME", &p) {
                Ok(validated) => Some(validated),
                Err(e) => {
                    warn!("Invalid XDG_DATA_HOME environment variable: {}. Falling back to system default.", e);
                    None
                }
            }
        });
...
pub fn data_dir() -> Result<PathBuf> {
    if is_custom_paths_allowed() {
        if let Ok(custom_path) = std::env::var("XDG_DATA_HOME") {
            warn!(
                "KODEGEN_ALLOW_CUSTOM_PATHS is enabled - bypassing validation for XDG_DATA_HOME (UNSAFE)"
            );
            return Ok(PathBuf::from(custom_path).join("kodegen"));
    ...
    if let Some(validated_path) = validated {
        ...
        dirs::data_dir()
            .map(|d| d.join("kodegen"))
            .or_else(|| dirs::home_dir().map(|h| h.join(".local/share/kodegen")))
            .ok_or_else(|| anyhow!("Cannot determine data directory"))
    }
}
...
pub fn log_dir() -> Result<PathBuf> {
    if is_custom_paths_allowed() {
        if let Ok(custom_path) = std::env::var("LOCALAPPDATA") {
            ...
            return Ok(PathBuf::from(custom_path).join("kodegen").join("logs"));
        }
    }
    ...
    if let Some(validated_path) = validated {
        Ok(validated_path.join("kodegen").join("logs"))
    } else {
        dirs::data_local_dir()
            .map(|d| d.join("kodegen").join("logs"))
            .ok_or_else(|| anyhow!("Cannot determine log directory"))
    }
}
...
pub fn log_dir() -> Result<PathBuf> {
    ...
    dirs::home_dir()
        .map(|h| h.join("Library/Logs/kodegen"))
        .ok_or_else(|| anyhow!("Cannot determine log directory"))
}

#[cfg(not(any(target_os = "windows", target_os = "macos")))]
pub fn log_dir() -> Result<PathBuf> {
    // Unix/Linux: logs subdirectory within state directory
